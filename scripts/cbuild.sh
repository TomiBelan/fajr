...
## configuration
MAIL_TESTS_TO="fajr-devel+postcommit@googlegroups.com"
...
function print_test_diff {
  WWW_ROOT='http://fks.sk/~ppershing/fajr_head/report'
  OLD="$1"
  NEW="$2"
  REV="$3"
  ...
  echo "There were some changes affecting failing tests in the fajr HEAD revision. Details:"
  echo "--------"
  echo "$DIFF_DATA"
  echo "--------"
...
if ! $DIFF_CMD "$OLD_TEST_REPORT" "$NEW_TEST_REPORT" ; then
  ...
  print_test_diff "$OLD_TEST_REPORT" "$NEW_TEST_REPORT" "$SVN_REVISION" | \
    mail -s "Failed tests changed in fajr r$SVN_REVISION" $MAIL_TESTS_TO \
