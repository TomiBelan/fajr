...
FULL_BASE="$PWD"
CODING_STANDARD="$FULL_BASE/scripts/phpcs/Fajr"
...
if [ $? -eq 0 ] ; then
    ...
    date >> $FILE;
    echo -e "\n" >> $FILE;
    phpcs --standard="$CODING_STANDARD" . --report=full --ignore=report | tr '<>' '()' >> $FILE;
    echo -e "\n\n\n++++++++++++++++++++++++++++++++++++++++++++++++++\n\n\n" >> $FILE;

    echo "Second pass (statistics)"
    phpcs --standard="$CODING_STANDARD" . --report=summary --ignore=report | tr '<>' '()' >> $FILE;
    echo "</pre></html>" >> $FILE;
