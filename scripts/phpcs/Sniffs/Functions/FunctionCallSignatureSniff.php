 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class Fajr_Sniffs_Functions_FunctionCallSignatureSniff implements PHP_CodeSniffer_Sniff
{
            ...
            if ($tokens[$i]['line'] !== $lastLine) {
                ...
                */
                ...
                if ($secondLineIndent == -1) {
                  $secondLineIndent = $foundIndent;
                }
                $expectedIndent = $secondLineIndent;

                if ($expectedIndent !== $foundIndent) {
                    $error = "Multi-line function call not indented correctly; expected $expectedIndent spaces but found $foundIndent";
                    $phpcsFile->addError($error, $i);
                }
                if ($foundIndent < $functionIndent + 4) {
                    $error = "Multi-line function call not indented by at least 4 spaces; expected "
                    . ($functionIndent+4) . " spaces but found $foundIndent";
                    $phpcsFile->addError($error, $i);
                }
            }//end if
        }//end for
